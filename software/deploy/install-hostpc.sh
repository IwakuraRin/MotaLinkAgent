#!/usr/bin/env bash
# 在已安装的 Ubuntu 20.04 Server（或其它 systemd 发行版）上安装 HostPC（Go + Vue 静态资源）。
# 用法（需 root）：
#   在仓库中先构建：pnpm -C software/frontend install && pnpm -C software/frontend run build && go -C software/backend build -o hostpc .
#   sudo software/deploy/install-hostpc.sh
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
BIN_SRC="$ROOT/backend/hostpc"
...
if [[ ! -x "$BIN_SRC" ]]; then
  echo "缺少可执行文件: $BIN_SRC — 请先执行: (cd $ROOT/backend && go build -o hostpc .)" >&2
  exit 1
fi
