...
// (Build frontend first: cd ../frontend && pnpm install && pnpm build)
//
// Wails: this stack is browser-first for 局域网; you can embed the same `frontend/dist`
...
func main() {
	...
	staticDir := flag.String("static", "../frontend/dist", "path to Vue build output (pnpm build in software/frontend)")
	...
	if err != nil || !st.IsDir() {
		log.Fatalf("static dir not found: %s — run: cd ../frontend && pnpm install && pnpm build", *staticDir)
	}
	...
	if updateScriptPath == "" && repoRootTrim != "" {
		for _, p := range []string{
			filepath.Join(repoRootTrim, "software", "deploy", "hostpc-self-update.sh"),
			filepath.Join(repoRootTrim, "OmniControlPanel", "deploy", "hostpc-self-update.sh"),
			filepath.Join(repoRootTrim, "OmniOS", "deploy", "hostpc-self-update.sh"),
			filepath.Join(repoRootTrim, "OmniOS", "hostpc", "deploy", "hostpc-self-update.sh"),
			filepath.Join(repoRootTrim, "HostPC", "deploy", "hostpc-self-update.sh"),
		} {
