...
function readMainTab(): 'console' | 'ssh' | 'files' {
  if (typeof sessionStorage === 'undefined') return 'console'
  const v = sessionStorage.getItem('omniroam.main_tab')
  if (v === 'files') return 'files'
  if (v === 'ssh' || v === 'desktop') return 'ssh'
  return 'console'
}

const mainTab = ref<'console' | 'ssh' | 'files'>(readMainTab())
...
<template>
  ...
  >
    ...
    >
      ...
      <button
        type="button"
        class="border-b-2 px-4 font-mono text-xs font-semibold transition-colors"
        :class="
          mainTab === 'ssh'
            ? 'border-pve-accent text-white'
            : 'border-transparent text-pve-muted hover:text-pve-text'
    ...
    <!-- -------- 模块：主内容区（v-show 保持 ssh 终端挂载，切换 Tab 不断开） -------- -->
    <div class="flex min-h-0 flex-1 flex-col">
      ...
      <div
        v-show="mainTab === 'ssh'"
        class="flex min-h-0 min-w-0 flex-1 flex-col bg-[#0c0e12] p-2"
      >
        <DeskTerminal class="h-full min-h-0 flex-1 rounded border border-white/10" />
      </div>
      <div
        v-show="mainTab === 'files'"
        class="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden p-2"
      >
        <DeskFiles class="h-full min-h-0 min-w-0 flex-1" />
      </div>
    </div>
