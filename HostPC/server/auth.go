...
func ensureDefaultWebUser(ctx context.Context, db *sql.DB) error {
	...
	hash, err := bcrypt.GenerateFromPassword([]byte("change-me-on-first-login"), bcrypt.DefaultCost)
	if err != nil {
		return err
	}
	...
	log.Println("auth: created default web user \"user\" / change-me-on-first-login (must change password on first login)")
	return nil
}
